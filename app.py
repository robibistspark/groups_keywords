...
def parse_group(group_id):
    ...
    stops.extend(["это", "всё", "который", "весь", 'мочь', 'свой', "ваш", 
                  "твой", "год", "амит", 'также'])
...
def process():
    ...
    return render_template(
        'result.html', similar_groups=similar,
...
def file():
    ...
    posts_query = f'''
    select id_group, post_text from posts
    ...
    return send_file(
        mem,
        as_attachment=True,
        attachment_filename=f'{group_id}.csv',
        mimetype='text/csv'
    )
